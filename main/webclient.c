...
// see if it is !=
	...
	if  ((header.members.mArr[METADATA] == NULL)||((header.members.mArr[METADATA] != NULL)&&(t!= NULL)&&(strcmp(tt,header.members.mArr[METADATA]) != 0)))
	{
		...
		if(strlen(t)>3) //Если у нас вообще в буфере более 3х символов. Вдруг это ЧИЖ в CP-1251 кодировке? Ошибки нам ни к чему.
			if(t[0]==0xEF)
				if(t[1]==0xBB)
		...
		if (b != NULL) //Если память выделилась
		{
			...
			while (index < tLen)  //Организуем цикл. Пока входящая строка не закончится, будем перебирать её побайтно. 
			{
				...
				{  
					...
					{
						if(chr == 0xA8) chr = 177; //Если обнаружили букву "Ё", то у нее своя зависимость. Подгоняем ту цифру, которая позволит на выходе получить нужную последовательность UTF-8 = 0xD0,0x81
						if(chr == 0xB8) chr = 257; //Если обнаружили букву "ё", то у нее своя зависимость. Подгоняем ту цифру, которая позволит на выходе получить нужную последовательность UTF-8 = 0xD1,0x91
